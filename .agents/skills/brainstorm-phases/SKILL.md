...
| `cluster` | option pool ≥ 8 across rounds; before converging | drag field; distance IS data | `positions`/`clusters`: the user's implicit taxonomy — name the clusters back to them; the digest now carries a SPATIAL READ of the raw positions (cluster tightness, closest cross-cluster pair, outliers — see Structured intent channels below); `gapNotes`: HIGHEST-VALUE SIGNAL — generate the hybrid living between those clusters next round |
...
## Structured intent channels (handoff fidelity)

INTERPRET — new response fields, each folded into the next round:

- **`response.questionAnswers`** — answers to YOUR `Board.questions` (mid-round clarifiers),
  keyed by question id; the digest carries them as `Answer — "<question>": …` lines. Fold
  each answer into the next round's direction like a note — you asked, they answered, the
  round must show it.
- **`response.uncertainties`** — option ids the user toggled "unsure". **UNSURE ≠ KILL**
  (the digest says so explicitly): next round include a clarifying variant of each, or ask
  a `Board.questions` clarifier probing WHY it's hard to judge. Never silently drop an
  unsure option.
- **`response.remixNotes`** — remix RECIPES keyed `"a×b"`, riding beside `remixPairs`; the
  digest appends `Recipe (what to take from each): …` to the remix line. Obey the
  attribute split literally — "layout from A, palette from B" is an EXECUTABLE
  instruction, not a vibe: the offspring takes exactly the named attribute from each parent.
- **`response.optionAnnotations` + `annotated-<id>.png` attachments** — marks drawn ON an
  option's SVG (the fullscreen Annotate mode). The digest emits `Annotated ON "<label>"`
  lines with mark counts, note texts, and a `VIEW <savedPath>` pointer. VIEW the png
  (vision — the marks in place) AND traverse the structured field
  (the saved round response file's `optionAnnotations[id]`: coordinates + palette color names):
  arrows point AT the element to change (tail→head — the HEAD is the target), notes are
  literal instructions. Same reading vocabulary as the `reading-scribbles` skill, applied
  per option.
- **Positions spatial narrative** — the digest now reads the raw `positions` drag geometry
  into lines, not just the discrete cluster sets: per-cluster tightness (**welded** = treat
  as ONE fused direction; close; **loose** = related but distinct ideas), the closest
  CROSS-cluster pair (the user ALMOST joined these — a hybrid of the two is a low-risk
  bet), spatial outliers (parked far from everything — uniquely valued or unclassifiable;
  probe with a clarifying variant or question, DON'T drop), and near pairs when the user
  arranged without forming clusters (treat each pair as an implicit cluster).

AUTHOR — new board-side channels (both optional, never gating):

- **`questions` on `present_board`** (0–4, the SurveyQuestion shape) when a signal was
  ambiguous — rendered as the "Claude asks" box beside the options; answers return in
  `questionAnswers`. Ask about the ambiguity, not the obvious.
- **`rationale` on each option, from round 2 on** — 1–2 sentences QUOTING the user
  feedback it responds to; the studio renders it under the option (beside its lineage
  chips) so the human SEES their feedback driving the round. Brief the artisan to write it.

## Changelog
- 2026-07-09 — structured intent channels: interpret questionAnswers / uncertainties (unsure
  ≠ kill) / remixNotes (executable recipes) / optionAnnotations (+ annotated-<id>.png, VIEW
  + traverse) / the digest's positions spatial narrative (welded clusters, cross-cluster
  bridge pairs, outliers); author mid-round Board.questions + per-option rationale quoting
  the feedback (from handoff-fidelity-2026-07-09)
