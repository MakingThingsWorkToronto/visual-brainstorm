...
- **Blank-canvas scribbles have NO photo layer.** The folder's README says which surface was
  marked ("annotated photo" vs "annotated blank canvas"), and `scribble.json`'s
  `background.present` is `false`; there is no `photo.*` file — don't try to Read one. The marks
  themselves ARE the subject: read the drawn shapes + notes as a freehand sketch brief.
...
## Changelog
- 2026-07-09 — blank-canvas variant: no photo layer exists (`background.present:false`, no
  `photo.*` file) — the marks are the subject; README names the surface (from
  photo-scribble-annotation-2026-07-09 review: README used to claim a photo that wasn't there)
