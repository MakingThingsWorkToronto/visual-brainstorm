...
| 4 | **Extract the runtime adapter seam in MCP** | Refactor `apps/mcp` so the existing Claude orchestration path is treated as one engine adapter over the same MCP/studio backbone. `preview` stays a dumb fixtures harness. The seam must make it clear what a second real engine would need to implement without duplicating bridge/session logic. | completed |
...
| 6 | **Make the agentic architecture CODEX-referenceable** | Add the docs/registry/contracts a future CODEX adapter would need: authoritative surface map, engine/model schema, workflow-to-tool mapping, and harness expectations. If a concrete CODEX customization format is available, scaffold the wrapper layer; if not, land the referenceable architecture and mark the actual wrapper runtime `blocked-external` rather than guessing. | completed |
| 7 | **Verification across real and dumb harnesses** | Prove the refactor did not break the product: `npm run build`, `npm test`, and real human verification for the live Claude path. Also verify the preview harness stays dumb and honest. Copilot verification: the repo proves the registry → adapter-map → prompt/agent chain and the existing MCP/runtime tool path; slash-menu exposure in Copilot Chat is a VS Code host spot-check, reported honestly rather than faked. If CODEX adapter work is still external-blocked, report exactly what is verified vs deferred. | completed |
| 8 | **Docs + authority reconciliation** | Update README and the relevant wiki pages with the authoritative architecture: what `.claude` owns, what Copilot owns, what future CODEX would own, what remains local-only, how engines/providers/models are represented, and what the current support matrix actually is. Append `wiki/log.md` for every wiki edit. | completed |
...
- 2026-07-08 — BUILD slice landed for phase 4: `apps/mcp` now centralizes runtime-specific behavior in `src/engine-adapter.ts`, `config.ts` and `Bridge` both normalize legacy model inputs through that adapter instead of duplicating Claude-only inference, `index.ts` instantiates the bridge from an explicit engine adapter, and focused validation passed for custom-runtime config + `/api/state` behavior.
- 2026-07-08 — verification after the phase-4 seam passed: `npm run build` and `npm test` both succeeded, including the real bridge smoke, UI smoke, and human-sim archived/live journeys.
- 2026-07-08 — BUILD/docs slice landed for phases 7 and 8 except CODEX: added `tests/copilot-adapter.test.mjs` to prove `.claude/agentic-surface-registry.json` stays aligned with `.github/agentic-surface-registry.json` and the on-disk prompt/agent wrappers; README + user-guide + testing-observability now state the honest boundary that Copilot slash-menu surfacing is VS Code host behavior, while the repo verifies the adapter chain and shared MCP/runtime path.
- 2026-07-08 — final verification pass landed: hardened the browser harnesses (`scripts/human-sim.mjs`, `scripts/lib/cdp.mjs`) against local CDP/startup timing flake, then reran `npm run build` and the full `npm test` stack to green, including smoke, UI smoke, live human-sim, and archived human-sim.
- 2026-07-09 — closeout landed: phase 6 is satisfied by the now-documented provider-neutral registry, engine/model schema, workflow-to-tool adapter map, and per-harness wiki pages; no concrete CODEX host format exists yet, so the actual CODEX wrapper remains honestly external rather than guessed. Reconciled the live Copilot adapter state at closeout by indexing `read-mindmap` and `read-scribble`, recording intentional Copilot exclusions for `add-theme` / `revisit-round`, and rerunning the focused harness proof to zero guard warnings.