...
- 2026-07-07 00:06 — token-meter: DiscussionSummary.tokens + StudioState.tokens, SessionStore.tokenTotals + list() sums from progress.jsonl, bridge /api/state + /api/discussions(+/<id>) exposure, pipe-progress transcript-delta cursor (Stop/SubagentStop), studio Σ badge + sidebar per-thread counts; verify: npm run build + npm test → all pass (57 unit, smoke exact-delta, ui-smoke); commits: bulk swept into concurrent closeout 59738f7, Stop-hook remainder 81c63af
