...
**Status:** open — authority registry + Copilot adapter slices landed; provider-aware protocol/config, studio model seam, MCP engine-adapter extraction, Copilot verification, and docs reconciliation landed; CODEX-referenceable closeout still pending
...
- 2026-07-08 — BUILD/docs slice landed for phases 7 and 8 except CODEX: added `tests/copilot-adapter.test.mjs` to prove `.claude/agentic-surface-registry.json` stays aligned with `.github/agentic-surface-map.json` and the on-disk prompt/agent wrappers; README + user-guide + testing-observability now state the honest boundary that Copilot slash-menu surfacing is VS Code host behavior, while the repo verifies the adapter chain and shared MCP/runtime path.
