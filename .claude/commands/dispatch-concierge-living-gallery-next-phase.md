...
4. **Ship** — first `git log --oneline -3`: a concurrent session's commit may have already
   ...
   Subject: `feat(concierge-living-gallery-<phase-tag>): <one line>`.
5. **Persist** — flip the row to `done`, append the Progress log line
   ...
   `git commit --only discussion/concierge-living-gallery-2026-07-07/plan.md -m "chore(concierge-living-gallery): tick <phase-tag>"`.
...
`[dispatch concierge-living-gallery HH:MM] <phase-tag>: done <hash> | in-progress <reason> | blocked <reason> | ALL PHASES COMPLETE → closeout`
