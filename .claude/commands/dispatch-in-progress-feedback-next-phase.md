...
# /dispatch-in-progress-feedback-next-phase — one loop tick for "In-Progress Feedback — live status, artifact keep/kill, and the kill→regenerate loop"

Plan: `discussion/in-progress-feedback-2026-07-09/plan.md` — the ONLY state. Read it fresh every tick;
...
4. **Ship** — first `git log --oneline -3`: a concurrent session's commit may have already
   ...
   Subject: `feat(in-progress-feedback-<phase-tag>): <one line>`.
5. **Persist** — flip the row to `done`, append the Progress log line
   ...
   `git commit --only discussion/in-progress-feedback-2026-07-09/plan.md -m "chore(in-progress-feedback): tick <phase-tag>"`.
...
`[dispatch in-progress-feedback HH:MM] <phase-tag>: done <hash> | in-progress <reason> | blocked <reason> | ALL PHASES COMPLETE → closeout`
