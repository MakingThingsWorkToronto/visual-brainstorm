...
| 22b | Artifact keep/kill verdict (fullscreen viewer Keep/Kill button) | `POST /api/artifact-verdict {artifactSlug, verdict, note?}` | bridge (deterministic endpoint: 200 response + pending-replacements.json rewrite) + routes kill→replace to orchestrator via command `replace-artifact` queued (same channel as artifact-chat); replacedBy sidecar rewrite on next `capture_artifact {replaces}` | kill→replace is orchestration (svg-artisan draws from characteristic + note); live status: pending-replacement placeholder (testid) with "↻ replacing…" shimmer |
...
| 7c | Artifact verdict (keep/kill + note) | `artifacts/<slug>.json` rewritten in place (verdict + verdictNote + verdictAt fields) + `brainstorm.md` appends `> Artifact verdict (slug): KEEP|KILL — note` line | thread reload; live `artifact` broadcast; kill→replace pending routed via `pending-replacements.json` + `StudioState.pendingReplacements` |
...
| 12f | Pending replacements (in-flight kill→replace) | `<thread>/pending-replacements.json` (whole-array atomic rewrite: `PendingReplacement[]` with replacesSlug, characteristic, note, at; cleared per entry on capture with `replaces`) | `StudioState.pendingReplacements`; studio renders shimmer placeholder (testid pending-replacement) with ↻ replacing… until capture completes; persistent across reload via SessionStore.open |
