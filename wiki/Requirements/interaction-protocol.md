...
**Intake log — permanent chat history.** The entire intake (brief, concierge answers, gallery pick) is logged as a discriminated union `IntakeLogEntry[]` persisted to `<thread>/intake-log.json` (atomic whole-array write; reloaded by `SessionStore.open`). Protocol: `IntakeLogEntrySchema` with discriminator `kind`: (1) `kind:'brief'` — `prompt` (composed brief sent to orchestrator), optional `rawBrief` (exactly what user typed), `answers: {id?, question, answers[]}[]` (keyed by SurveyQuestion id), optional `model`; (2) `kind:'concierge'` — `question`, `answer` (never empty in logged entry), `picked` (tapped chips), `typed` (user's own words); (3) `kind:'gallery-pick'` — `method`, `offered` (roster at pick time). Each entry carries `at` (ISO string). `StudioState.intakeLog: IntakeLogEntry[]` and `ServerToStudio` carries `{type:'intake', entry}` on every append. Timeouts are NOT logged (no user message — rule 6). `recordBrief` logs the submitted New Discussion brief and returns the entry; `recordConcierge` and `recordGalleryPick` also append structured entries and return them (brainstorm.md text unchanged). The studio's new `IntakeHistory` component renders the intake log as permanent timeline bubbles FIRST in any timeline (live or archived): brief as a user bubble with structured answers + "✎ revise this brief" button (live only; reopens New Discussion prefilled from `rawBrief` + answer `id`s; sending dispatches a fresh new-brainstorm), each concierge exchange as question/answer bubbles, gallery pick as a marker line.

**Snap-back fix — once intake begins, timeline never yields back to the panel.** The `landing` condition (which used to swap the whole surface for the New Discussion panel whenever no live surface was present) now ALSO requires `intakeLog.length === 0`, so once intake begins the timeline remains rendered. Gaps between intake stages show the `intake-preparing` working shimmer instead. The New Discussion panel is now a timeline block, not a surface takeover: when open, the rest of the timeline stays rendered below it and the WayfinderStrip stays visible.
